...
| **Real Groq call** | **Performed 2026-09-19 (local stack, free-plan key, fictional data).** `node scripts/ai-smoke.mjs --require` → HTTP 200 in 662 ms, parseable JSON, structural checks passed (the script printed its result and then hit a Node/libuv teardown assertion on Windows, exit code 127 — a runtime quirk after the result, not a provider failure). Authenticated end to end through the app: assistant answered with `aiGenerated=true, fallbackReason=NONE`; a prompt-injection request and a request for another customer's account were refused/answered only from the caller's own data and changed no balance; the on-demand category suggestion returned `source=AI`. Two design-intended degradations were also observed live: a model answer containing a figure the backend had not supplied was discarded (`UNVERIFIED_NUMBER`), and rapid repeated calls were declined locally (`LOCAL_QUOTA`, 6,000 estimated tokens/minute) with the labelled fallback. **Provider failure, tested separately** with a deliberately invalid key: the assistant returned the labelled "Calculated answer" (`PROVIDER_AUTH`), the AI category suggestion returned a clean 422, and banking operations were unaffected. **Repeated on the public deployment (2026-09-19, replacement key after the first was revoked):** genuine assistant and categorization responses, injection/cross-customer probes handled, an unsupported model figure rejected by the backend validator, and the labelled fallback with a deliberately rejected key. Not verified: behaviour on a real Groq 429/5xx (covered only by simulated-provider tests). |
